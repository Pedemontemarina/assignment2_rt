...
class Control(Node):
    ...
    def publish_obstacle_info(self):
        ...
        # publish distance from fixed point
        msg_dist = Distance()
        msg_dist.distance = self.distance
        self.publisher_fix.publish(msg_dist)

    # callback of odometry, pubblish every time position changes
    def compute_fixed_obstacle_distance(self,msg:Odometry):
